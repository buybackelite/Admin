...
export default function PriceEngine() {
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowAdd(true)}
            className="flex items-center gap-2 px-4 py-2 gradient-admin text-white rounded-xl hover:shadow-lg hover:shadow-emerald-500/20 transition-all font-medium text-sm">
      ...
      {msg && (
        <div className={`p-3 rounded-xl text-sm font-medium ${msg.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
          {msg.text}
        </div>
      )}
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              ...
              <select value={addForm.device_type} onChange={(e) => setAddForm({ ...addForm, device_type: e.target.value })}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 text-sm bg-gray-50">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search models..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 outline-none text-sm bg-gray-50" />
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-5 py-3 text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Model</th>
                    <th className="text-left px-5 py-3 text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Category</th>
                    <th className="text-right px-5 py-3 text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Base Price</th>
                    <th className="text-right px-5 py-3 text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                      ...
                      <tr key={model.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-5 py-3.5">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium ${
                            model.device_type === 'MacBook' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'
                          }`}>
                          ...
                          {editingId === model.id ? (
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              <input type="number" value={editPrice} onChange={(e) => setEditPrice(e.target.value)}
                                className="w-28 px-2.5 py-1.5 border border-emerald-300 rounded-lg text-right text-sm font-semibold outline-none focus:ring-2 focus:ring-emerald-500/30 bg-emerald-50/50" autoFocus />
                              <button onClick={() => handleEdit(model.id)} className="p-1.5 bg-emerald-100 text-emerald-600 rounded-lg hover:bg-emerald-200 transition-colors">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          model.device_type === 'MacBook' ? 'bg-indigo-50' : 'bg-emerald-50'
                        }`}>
                          {model.device_type === 'MacBook' ? <Laptop className="w-4 h-4 text-indigo-600" /> : <Tablet className="w-4 h-4 text-emerald-600" />}
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        {editingId === model.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => handleEdit(model.id)} className="p-1 bg-emerald-100 text-emerald-600 rounded-lg"><Save className="w-3 h-3" /></button>
                          ...
                          <>
                            <span className="font-bold text-sm text-gray-900">₹{basePrice.toLocaleString('en-IN')}</span>
                            <button onClick={() => { setEditingId(model.id); setEditPrice(numToInt(model.base_price).toString()) }}
                              className="p-1.5 text-gray-400 hover:text-emerald-600 rounded-lg"><Edit2 className="w-3.5 h-3.5" /></button>
                            <button onClick={() => handleDelete(model.id, model.model_name)}
      ...
      <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4">
        <h3 className="font-bold text-emerald-800 mb-2 text-sm">How Pricing Works</h3>
